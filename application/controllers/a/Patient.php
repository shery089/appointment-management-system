...
//		$this->layouts->view('templates/admin/patients', $data);
	...
	public function add_patient_lookup()
	{
	    ...
	    $this->form_validation->set_rules(
	    		...
	    		'trim|required|min_length[3]|max_length[70]',
	        	array(
                	'required'      => 'You have not provided any %s.'
        		)
	    );
		...
		$this->form_validation->set_rules(
				...
				'cnic', 'Cnic',
				'trim|required|callback__cnic_regex',
		    	array(
		        	'required'      => 'You have not provided any %s.'
				)
	    );  

	    $this->form_validation->set_message('_cnic_regex', 'Please provide a valid CNIC with dashes');
	    ...
	    else
	    {
			...
			{
				$this->session->set_flashdata('success_message', 'Patient with MR_Number ' . ucfirst($this->input->post('mr_number')) . ' has been successfully Added!');
			    redirect('/a/patient/');
	    	}
	    	else
	    	{
	...
	{		
	    ...
	    {
			if($this->patient_model->update_patient($id))
			{
				$this->session->set_flashdata('success_message', 'Patient with MR_Number ' . ucfirst($this->input->post('mr_number')) . ' has been successfully updated!');
		    	redirect('/a/patient/');
			}
			else
			{
				$this->layouts->view('templates/admin/edit_patient');
			}
	    }		
	}
