...
class Appointment_model extends CI_Model {
    ...
    {         
        ...
        elseif(!empty($this->input->post('doctor_old')))
        {
            $this->doctor_id = html_escape(strtolower(trim($this->db->escape($this->input->post('doctor_old')), "''")));                
        }
        else
        {
        ...
        {
            $this->morning_shift = $this->trim_time(html_escape(trim($this->input->post('morning_shift_2'))), "''");
        }

        if(!empty($this->input->post('evening_shift')))
        {
            $this->evening_shift = $this->trim_time(html_escape(trim($this->input->post('evening_shift'))), "''");
        }
        else
        {
            $this->evening_shift = $this->trim_time(html_escape(trim($this->input->post('evening_shift_2'))), "''");
        }
        
        if(!empty($this->input->post('visit_purpose')))
        {
            $this->visit_purpose = html_escape(trim($this->db->escape($this->input->post('visit_purpose')), "''"));
        }
        else
        {
            $this->visit_purpose = html_escape(trim($this->db->escape($this->input->post('visit_purpose_2')), "''"));
        }

        if(!empty($this->input->post('submitted_date_1')))
        {
            $this->date = html_escape(trim($this->db->escape($this->input->post('submitted_date_1')), "''"));
        }
        else
        {
            $this->date = html_escape(trim($this->db->escape($this->input->post('submitted_date_2')), "''"));
        }
